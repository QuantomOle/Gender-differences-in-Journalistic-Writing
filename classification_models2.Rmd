...
set.seed(123)
svm.radial  <- train(author_gender ~ ., data=dfTrain, trControl = ctrl, method = "svmRadial")
...
# tuning parameters: sigma, C 
set.seed(123)
svm.tfidf.radial  <- train(author_gender ~ ., data=dfTrain_w, trControl = ctrl, method = "svmRadial")
...
svm.radial.predict <- predict(svm.radial,newdata = dfTest)

svm.tfidf.radial.predict <- predict(svm.tfidf.radial,newdata = dfTest_w)
...
confusionMatrix(svm.radial.predict, as.factor(dfTest$author_gender)) # radial kernel, unweighted

confusionMatrix(svm.tfidf.radial.predict, as.factor(dfTest$author_gender)) # radial kernel, weighted

# get execution time
svm.linear$times
svm.tfidf.linear$times
svm.radial$times
svm.tfidf.radial$times
```
...
#fit Lasso Regression using unweighted Train set
set.seed(123)
lasso <- train(
  author_gender ~., data = dfTrain, method = "glmnet",
  trControl = ctrl,
...
# predict on test data
lasso.predict <- predict(lasso, newdata = dfTest) 

lasso.tfidf.predict <- predict(lasso.tfidf, newdata = dfTest_w)

#confusion matrix
confusionMatrix(lasso.predict, as.factor(dfTest$author_gender))

confusionMatrix(lasso.tfidf.predict, as.factor(dfTest_w$author_gender))
...
```{r}
models <- list(Ridge = ridge, Ridge.tfidf = ridge.tfidf, Lasso = lasso, 
               Lasso.tfidf = lasso.tfidf, NaiveBayes = nb.fit, NaiveBayes.tfidf = nb.tfidf, RandomForest = rf.fit, RandomForest.tfidf = rf.tfidf, SVM.linear = svm.linear, SVM.tdidf.linear = svm.tfidf.linear, kNN = knn.fit, kNN.tfidf = knn.tfidf)
